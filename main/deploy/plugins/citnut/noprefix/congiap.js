		...
		async function handleReply(message) {
			if (message.type == "message_reply") {
				return {
					...
					reply: {
						messageID: message.messageReply.messageID,
						senderID: message.messageReply.senderID,
						body: message.messageReply.body
					}
				}
			}else { return false }
		};

		let event = await handleReply(message);

		if (!setting.run.congiap || message.type != "message_reply") {
		}else if (!setting.run.congiap && message.type == "message_reply") {
			reply("lệnh này đã bị tắt")
		}else if (event.reply.body == citnut && event.reply.senderID == fca.getCurrentUserID()) {
			switch(event.body){
				case "1":
					fca.sendMessage({
				...
				break;
				default:
					fca.sendMessage("Djtconmemaynguvailon nhap 1-12 thoi <3 iu", event.threadID, event.messageID); 
				break
			};
			asyncWait(10000).then(() => {fca.unsendMessage(event.reply.messageID)})
		};
